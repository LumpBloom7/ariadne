...
FloatMPBounds BernsteinPolynomial::factorial(int x) {
    ...
    auto res = factorial(x - 1);

    _mantissaBitsCount += log2(x);
    int numbits = static_cast<int> ((_mantissaBitsCount));

    res = FloatMPBounds(res, MultiplePrecision(numbits)) * x;

    std::cout << res << res.precision() << std::endl;

    return _factorialCache.emplace_back(res);
}
